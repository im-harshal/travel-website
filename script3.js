...
/**
 * Background color and font change common for all pages
 */
const colors = [
...
/**
 * Stays Form Validation and Result Display
 */
function validateAndSearchStay() {
  ...
  const allowedCities = ["Austin", "Dallas", "Houston", "San Antonio", "Los Angeles", "San Francisco", "San Diego", "Sacramento"];
  ...
  if (checkOut < minDate || checkOut > maxDate || checkOut <= checkIn) {
    message += "Check-out date must be after check-in and within the same date range.<br>";
    isValid = false;
  }

  // Guest count validation and rooms calculation
